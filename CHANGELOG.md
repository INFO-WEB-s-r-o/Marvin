...
- **Theme toggle icon flash on light-mode users** — `ThemeProvider` initialised React state as `'dark'` and read localStorage in a layout effect, causing a re-render that briefly showed the wrong toggle icon for light-theme users. Replaced with a lazy `useState` initializer that reads localStorage during initialization, eliminating the extra render entirely. (fixes #397)
