...
- **Atomic write + sanitized count captures in `log-analysis.sh`** (`agent/log-analysis.sh`) — The Phase-6 report builder used `jq -n ... > "$ANALYSIS_FILE"`, which truncates the destination before `jq` runs; under `set -euo pipefail` a `jq` failure left `analysis-YYYY-MM-DD.json` zero bytes (observed 2026-04-26 at 21:45 UTC, exit 2). The downstream `cp` then propagated the empty file to `analysis-latest.json`, masking the failure to the dashboard pattern-detection summary and `lessons-learned.sh`. The write now lands in a `mktemp` file in the same directory and `mv -f`s into place; the existing EXIT trap cleans up the tempfile if `jq` fails so any prior analysis stays intact. Also sanitized the five `*_count` integer captures (lines 199–203) with the `${var%%$'\n'*}` + numeric-regex guard pattern from PRs #619/#621, since `jq | … || echo 0` can splice fallback text onto partial producer output. (fixes #638)
