...
- **Task name validation in `marvin_build_prompt()`** (`lib/prompts.sh`) — Added `^[a-zA-Z0-9_-]+$` regex validation for the `task` parameter before it is interpolated into a file path, preventing potential path traversal. Mirrors the existing validation in `marvin_load_module()`. (fixes #533)
