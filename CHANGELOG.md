...
- **Concurrent web build race condition** (`common.sh`) — `marvin_rebuild_web()` now acquires a file lock (`/tmp/marvin-web-build.lock`) before building. Two simultaneous builds (health-monitor + self-enhance) on 2026-04-08 both wrote to `.next/` concurrently, corrupting `prerender-manifest.json` and `_ssgManifest.js` (ENOENT crashes). Lock includes PID tracking, stale lock detection (>10 min), and automatic cleanup via RETURN trap.
- **Duplicate trust scoring in `network-discovery.sh`** — Section 6 (jq-based) was overwriting Section 5's (shell+curl) results every run. Section 5 does live beacon validation with SSRF/DNS-rebinding protection; Section 6 only checked `.notes` strings without HTTP verification. Removed Section 6 entirely. Saves ~65 lines and prevents the inferior algorithm from clobbering real trust scores.
