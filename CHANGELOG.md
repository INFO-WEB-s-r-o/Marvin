...
- **Replace hardcoded `flock` FD 9 with bash auto-allocated FD** (`agent/lib/claude.sh`) — Used `exec {CLAUDE_LOCK_FD}>"$CLAUDE_LOCK_FILE"` so bash picks a free file descriptor at runtime instead of relying on a magic FD 9. Eliminates a latent risk where another sourced library or future helper could open FD 9 for an unrelated purpose and silently clobber the Claude concurrency lock state. Also marked `CLAUDE_LOCK_FD` as `local` so it doesn't leak into the caller's shell. (fixes #604)
