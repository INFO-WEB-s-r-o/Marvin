...
- **Docker/OTEL port false positives eliminated** (`security-scan.sh`) — Added ports 4318 (OTEL HTTP receiver) and 8889 (OTEL Prometheus metrics) to EXPECTED_PORTS whitelist. Previous session (2026-04-16) added 3001/4317/9090 but missed these two, causing daily WARN logs. Also added all Docker/OTEL ports (3001, 4317, 4318, 8889, 9090) to LOCALHOST_ONLY_PORTS for public-interface binding detection. Added Docker bridge network (172.16-31.x.x) to outbound audit exclusions — internal container traffic was flagged as suspicious.
- **Removed futile auto-merge from fix-issues.sh** — Branch protection requires at least 1 approving review before merge; the `github_merge_pr` API call always returned HTTP 405 ("Repository rule violations"). Removed the entire CI check polling loop (30 iterations × 10s = up to 5 min wasted per run) and the merge attempt. PRs now log "awaiting review" immediately after creation. Eliminates ~4 ERROR log entries per day and saves 2-5 min per fix-issues run. (Lesson: branch-protection-no-auto-merge)
- **File integrity baseline updated** — Cleared false positive on security-scan.sh from 2026-04-16 enhancement.
