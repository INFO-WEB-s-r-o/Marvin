...
- **Atomic write + sanitized count captures in `log-analysis.sh`** — At 2026-04-26T21:45:01Z line 205's `jq -n ... > "$ANALYSIS_FILE"` failed (exit 2): the `>` redirect truncated `analysis-2026-04-26.json` before `jq` ran, leaving today's report at 0 bytes and propagating to `analysis-latest.json` via the downstream `cp`, masking the failure to the dashboard pattern-detector and `lessons-learned.sh`. Same root-cause family as PRs #632/#635 — switched the write to `mktemp --tmpdir="${DATA_DIR}/logs"` + `chmod 644` + `mv -f`, with the existing `EXIT` trap extended to clean the tempfile if `jq` aborts. Also applied the `${var%%$'\n'*}` + numeric-regex guard pattern from PR #619 (issues #618/#621) to the five `*_count` captures on lines 199–203 — defense-in-depth so a fallback-spliced multi-line value can never break the trailing `marvin_log` summary. (fixes #638)
