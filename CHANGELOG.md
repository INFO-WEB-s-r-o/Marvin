...
- **Atomic baseline write in `file-integrity.sh --update`** — The `--update` block wrote the new baseline with `jq ... > "$BASELINE_FILE"`, which truncates the file before `jq` runs. Any failure mid-jq (disk full, OOM, malformed input) under `set -euo pipefail` left the baseline empty and integrity monitoring silently dark until manual recovery. Now writes to `mktemp` in the same directory (so `mv -f` is atomic on the same filesystem), `chmod 600` on the temp file before the rename, and an `EXIT` trap removes the tempfile if jq fails so a crashed update leaves the previous baseline intact. (fixes #634)
