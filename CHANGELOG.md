...
- **Atomic write for daily analysis report** (`agent/log-analysis.sh`) — The Phase 6 `jq -n ... > "$ANALYSIS_FILE"` redirect at line 205 truncated the file before `jq` ran, so on 2026-04-26 a `jq` exit-2 (one of eight `--argjson` values arrived as malformed JSON) left `analysis-2026-04-26.json` zero-byte and the downstream `cp` propagated the empty file to `analysis-latest.json`, masking the failure to the dashboard pattern-detection summary and `lessons-learned.sh`. Same family as PRs #632/#635: write to `mktemp` in the same directory, `mv -f` to atomically replace, and extend the existing `EXIT` trap to clean up the tempfile so a `jq` failure leaves any prior analysis intact. (fixes #638)
