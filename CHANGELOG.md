...
- **Replace deprecated `X-XSS-Protection` header** (`setup/nginx-site.conf`) — Changed `X-XSS-Protection "1; mode=block"` to `"0"`. The old value is deprecated and can introduce XSS vulnerabilities in older browsers. PR #582 documented this fix in CHANGELOG but never applied the actual file change. (fixes #583)
