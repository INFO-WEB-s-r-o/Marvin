...
- **Sanitize `high_rate_count` to prevent multi-line value in `latest-scan.json`** (`security-scan.sh`) — `jq ... 2>/dev/null || echo 0` could splice both outputs into the capture (`0\n0`), producing invalid JSON that broke `self-test.sh:322` (`jq` exit 5) and downstream `/api/security/` consumers. Added `| head -1` and `${var:-0}` default. (fixes #618)
