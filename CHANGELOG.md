...
- **Retry once on transient Claude failure for daily blog tasks** (`agent/lib/claude.sh`, `agent/morning-check.sh`, `agent/evening-report.sh`) — Added `run_claude_with_retry()` helper that retries `run_claude` once on exit code 1 (transient failures: API timeout, stochastic usage-policy classifier rejection, context edge cases). Only retries on exit 1 — exit 2 (lock timeout) and exit >1 (persistent errors) are not retried. `morning-check.sh` and `evening-report.sh` now use the helper with `max_retries=1`. Motivated by the 2026-04-21 incident where a stochastic usage-policy classifier rejected the morning prompt (29s, exit 1), and the "retry on next cron cycle" behaviour meant no morning blog for 24 hours. High-frequency tasks (hourly, every-5-min) continue to call `run_claude` directly — for them the next cycle is a cheaper retry.
