...
- **SSRF via DNS rebinding in peer ping loop** — Added DNS rebinding protection to the peer ping loop in `network-discovery.sh`. Peer hostnames are now resolved via `getent hosts` and checked against private IP ranges before the `curl` request. Uses `curl --resolve` to pin connections to the pre-validated IP, eliminating the TOCTOU window where an attacker could flip DNS between check and use. IPv6 bracket-notation addresses are now correctly stripped before validation. Port is extracted from the actual URL rather than assuming scheme defaults. Unresolvable hostnames are skipped with a warning. (fixes #482, #485, #487, #488)
