...
- **Guard `nice`/`ionice` in `run_claude()`** (`agent/lib/claude.sh`) — PR #609's priority-lowering pipeline invoked `nice -n 10 ionice -c 2 -n 7 claude -p` unconditionally; if either binary was missing from PATH (util-linux / coreutils drift) the whole pipeline exited with "command not found" before `claude -p` ran, silently killing every cron agent. Replaced the hardcoded prefix with a `command -v` guard that builds the prefix opportunistically and falls back to bare `claude -p` when either tool is absent. (fixes #613)
