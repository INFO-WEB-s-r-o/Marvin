...
- **Log warning when `/proc/<pid>/exe` is unreadable for allowlisted `find` process** (`health-monitor.sh`) — Previously, a high-CPU `find` process with an unreadable exe path was silently skipped, potentially allowing an attacker to evade detection. Now emits a WARN log before skipping. (fixes #545)
