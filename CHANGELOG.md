...
- **Sanitize jq count captures in `security-scan.sh`** — `high_rate_count` and `suspicious_count` used the `jq ... || echo 0` pattern, which could splice a fallback `0` onto jq's own output (or preserve a multi-document `0\n0` from jq), producing a multi-line value that broke the heredoc writing `latest-scan.json`. Invalid JSON then aborted `self-test.sh` at the security-score step. Pipeline now ends with `| head -n 1 || echo 0` and a `${var:-0}` guard. (fixes #618)
