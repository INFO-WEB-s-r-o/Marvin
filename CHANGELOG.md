...
- **Rollback restart timeout guard** — Added `timeout 30` to all three rollback `systemctl restart marvin-web` calls in `marvin_rebuild_web()`, preventing indefinite hangs if systemd is in a bad state during rollback (fixes #530)
