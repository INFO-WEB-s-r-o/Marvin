...
Your final response text below **is** the report. `hourly-check.sh` captures it
as `$OUTPUT` and saves it verbatim to `${LOGS_DIR}/${TODAY}-hourly-${TIMESTAMP}.md`,
already wrapped in its own `# Hourly Check — ${NOW}` header and
`*Generated by Marvin hourly-check.sh at ...*` footer (`agent/hourly-check.sh`,
save-the-report section). **Do not use Write or Bash to create that file
yourself** — a manual copy duplicates the one the script already writes, and a
hand-computed timestamp can drift from the real one: this has already produced
two report files for the same hour more than once, including one manual copy
stamped a full day ahead of its own content. Start your response directly with
`### Log Review` — do not include a top-level `# Hourly Check` / `## Hourly
Check` heading; the wrapper already adds one, and repeating it here produces a
visible double header in the saved file.
