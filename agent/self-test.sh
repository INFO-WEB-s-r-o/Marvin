...
# --- 9d, third part: fail2ban jail.local drift (#939) -------------------------
# jail.local has no tracked file counterpart to diff like the nginx/systemd/
# deploy-hook pairs above — it's generated by a heredoc in setup/bootstrap.sh —
# so this follows the cron check's marker-extraction shape instead: pull the
# heredoc between its `cat >` opener and the first standalone EOF, then diff
# against the live file with comment-only and blank lines stripped from both
# sides (the postfix/dovecot stanzas that #938 added to bootstrap.sh carry
# their own inline comments, so a byte-for-byte diff would WARN on wording, not
# policy). An extraction that yields nothing WARNs naming itself instead of
# reporting a false PASS. Same WARN-only, read-only-diff contract as §9d above.
_jail_src="${MARVIN_DIR}/setup/bootstrap.sh"
_jail_live="/etc/fail2ban/jail.local"
if [[ ! -f "$_jail_src" ]]; then
    test_warn "config drift: bootstrap.sh source missing (${_jail_src})"
    ...
    if [[ -z "$_jail_generated" ]]; then
        ...
        if diff -q <(printf '%s\n' "$_jail_norm_src") <(printf '%s\n' "$_jail_norm_live") >/dev/null 2>&1; then
            test_pass "config in sync: fail2ban jail.local (bootstrap.sh ⇆ /etc/fail2ban/jail.local, comments ignored)"
        else
            test_warn "config drift: fail2ban jail.local — bootstrap.sh would not reproduce live /etc/fail2ban/jail.local (a rebuild could silently drop a jail, per #939)"
        fi
    fi
fi
