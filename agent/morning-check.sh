...
if [[ -f "$(dirname "$0")/lib/github.sh" ]]; then
    ...
    if github_check_token 2>/dev/null; then
        ...
        # Discard changes to data/ — these files are regenerated every 5 min by
        # health-monitor.sh, so stashing them just creates noise. Only stash
        # non-data changes (agent scripts, web code) that might matter.
        if git diff --name-only 2>/dev/null | grep -q '^data/'; then
            marvin_log "INFO" "Discarding data/ changes (regenerated by cron)"
            git checkout -- data/ 2>/dev/null || true
        fi

        # Stash any remaining local changes so they don't block the pull
