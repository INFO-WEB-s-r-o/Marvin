...
if [[ -f "$(dirname "$0")/lib/github.sh" ]]; then
    ...
    if github_check_token 2>/dev/null; then
        ...
        # Discard local data/ changes before pulling — these files are
        # regenerated every 5-15 minutes by health-monitor and update-website,
        # so stashing them is pointless (the working tree gets dirty again
        # between stash and pull, causing the pull to fail). Code files are
        # never modified between pulls, so this is safe.
        git checkout -- data/ 2>/dev/null || true
        git clean -fd data/ 2>/dev/null || true
        marvin_log "INFO" "Discarded local data/ changes (will be regenerated by cron)"
